...
class ReportService {
  ...
  private static async generatePDFFromDOCXBuffer(data: ReportData): Promise<Buffer> {
    ...
    try {
      ...
      // NOTE: DAST template uses native TOC (SDT) that refreshes via Google Docs/Word.
      // No manual page number patching needed.
    ...
    const templateKey = data.template ? getTemplateKey(data.template) : 'unknown';
      if (templateKey === 'dast') {
        ...
        try {
          ...
          console.warn('[Report Service] DAST DOCX to PDF conversion failed:', (e as any)?.message || e);
          } finally {
            try {
              fs.rmSync(tempDir, { recursive: true, force: true });
            } catch {
